...
def print_batch_stat(n_iter, train_batch_loss, train_batch_n_acc, train_batch_size):
    ...
    loss_str = (
        "batch loss: {:.6f}".format(train_batch_loss)
        if type(train_batch_loss) is float
        else "batch loss: {}".format(train_batch_loss)
    )
...
def print_eval_res(result_dict, use_mcc=None):
    ...
    iter_str = "\tEval"
    loss_str = (
        "loss: {:.6f}".format(eval_loss)
        if type(eval_loss) is float
        else "eval loss: {}".format(eval_loss)
    )
