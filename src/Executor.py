...
class Executor:
    ...
    def unit_test_train(self):
        with tf.Session() as sess:
            ...
            logger.info(
                "Model: {0}, start a new session!".format(self.model.model_name)
            )
    ...
    def generation(self, sess, phase):
        ...
        for gen_batch_dict in generation_gen:
            ...
            gen_batch_n_acc = float(
                sess.run(metrics.n_accurate(y=gen_batch_y, y_=gen_batch_y_))
            )  # float
    ...
    def train_and_dev(self):
        with tf.Session(config=self.tf_config) as sess:
            ...
            checkpoint = tf.train.get_checkpoint_state(
                os.path.dirname(self.model.tf_checkpoint_file_path)
            )
            if checkpoint and checkpoint.model_checkpoint_path:
                ...
                logger.info(
                    "Model: {0}, session restored!".format(self.model.model_name)
                )
            else:
                logger.info(
                    "Model: {0}, start a new session!".format(self.model.model_name)
            ...
            for epoch in range(self.model.n_epochs):
                logger.info(
                    "Epoch: {0}/{1} start".format(epoch + 1, self.model.n_epochs)
                )
                ...
                train_batch_gen = self.pipe.batch_gen(
                    phase="train"
                )  # a new gen for a new epoch
                ...
                for train_batch_dict in train_batch_gen:
                    ...
                    # if epoch == self.model.n_epochs-1:
                    # train_batch_P_list.append(train_batch_P)
