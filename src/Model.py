...
class Model:
    ...
    def __init__(self):
        ...
        name_max_n = name_pattern_max_n.format(
            self.max_n_days, self.max_n_msgs, self.max_n_words
        )

        name_pattern_input_type = "word_embed-{0}.vmd_in-{1}"
        name_input_type = name_pattern_input_type.format(
            self.word_embed_type, self.variant_type
        )

        name_pattern_key = "alpha-{0}.anneal-{1}.rec-{2}"
        name_key = name_pattern_key.format(
            self.alpha, self.kl_lambda_anneal_rate, self.vmd_rec
        )

        name_pattern_train = "batch-{0}.opt-{1}.lr-{2}-drop-{3}-cell-{4}-tmp"
        name_train = name_pattern_train.format(
        ...
        # paths
        self.tf_graph_path = os.path.join(
            path_parser.graphs, self.model_name
        )  # summary
        self.tf_checkpoints_path = os.path.join(
            path_parser.checkpoints, self.model_name
        )  # checkpoints
        self.tf_checkpoint_file_path = os.path.join(
            self.tf_checkpoints_path, "checkpoint"
        )  # for restore
        self.tf_saver_path = os.path.join(self.tf_checkpoints_path, "sess")  # for save
        os.makedirs(self.tf_saver_path, exist_ok=True)
    ...
    def _build_placeholders(self):
        with tf.name_scope("placeholder"):
            ...
            # init
            self.word_table_init = tf.placeholder(
                dtype=tf.float32, shape=[vocab_size, self.word_embed_size]
            )

            # model
            self.stock_ph = tf.placeholder(dtype=tf.int32, shape=[None])
            self.T_ph = tf.placeholder(
                dtype=tf.int32,
                shape=[
                    None,
                ],
            )
            self.n_words_ph = tf.placeholder(
                dtype=tf.int32, shape=[None, self.max_n_days, self.max_n_msgs]
            )
            self.n_msgs_ph = tf.placeholder(
                dtype=tf.int32, shape=[None, self.max_n_days]
            )
            self.y_ph = tf.placeholder(
                dtype=tf.float32, shape=[None, self.max_n_days, self.y_size]
            )  # 2-d vectorised movement
            self.mv_percent_ph = tf.placeholder(
                dtype=tf.int32, shape=[None, self.max_n_days]
            )  # movement percent
            self.price_ph = tf.placeholder(
                dtype=tf.float32, shape=[None, self.max_n_days, 3]
            )  # high, low, close
            self.word_ph = tf.placeholder(
                dtype=tf.int32,
                shape=[None, self.max_n_days, self.max_n_msgs, self.max_n_words],
            )
            self.ss_index_ph = tf.placeholder(
                dtype=tf.int32, shape=[None, self.max_n_days, self.max_n_msgs]
            )

            # dropout
            self.dropout_mel_in = tf.placeholder_with_default(
                self.dropout_train_mel_in, shape=()
            )
            self.dropout_mel = tf.placeholder_with_default(
                self.dropout_train_mel, shape=()
            )
            self.dropout_ce = tf.placeholder_with_default(
                self.dropout_train_ce, shape=()
            )
            self.dropout_vmd_in = tf.placeholder_with_default(
                self.dropout_train_vmd_in, shape=()
            )
            self.dropout_vmd = tf.placeholder_with_default(
                self.dropout_train_vmd, shape=()
            )
        ...
        with tf.name_scope("embeds"):
            with tf.variable_scope("embeds"):
                word_table = tf.get_variable(
                    "word_table", initializer=self.word_table_init, trainable=False
                )
                self.word_embed = tf.nn.embedding_lookup(
    ...
    def _create_msg_embed_layer_in(self):
        ...
        with tf.name_scope("mel_in"):
            with tf.variable_scope("mel_in"):
                ...
                if self.use_in_bn:
                    mel_in = neural.bn(
                        mel_in, self.is_training_phase, bn_scope="bn-mel_inputs"
                    )
    ...
    def _create_msg_embed_layer(self):
        ...
        def _for_one_trading_day(daily_in, daily_ss_index_vec, daily_mask):
            ...
            msg_ids = tf.constant(
                np.arange(0, self.max_n_msgs, dtype=np.int32).reshape(
                    self.max_n_msgs, 1
                )
            )
        ...
        with tf.name_scope("mel"):
            with tf.variable_scope("mel_iter", reuse=tf.AUTO_REUSE):
                ...
                mel_cell_f = tf.contrib.rnn.DropoutWrapper(
                    mel_cell_f, output_keep_prob=1.0 - self.dropout_mel
                )
                mel_cell_b = tf.contrib.rnn.DropoutWrapper(
                    mel_cell_b, output_keep_prob=1.0 - self.dropout_mel
                )
    ...
    def _create_corpus_embed(self):
        ...
        with tf.name_scope("corpus_embed"):
            ...
            with tf.variable_scope("u_t"):
                proj_u = self._linear(
                    self.msg_embed, self.msg_embed_size, "tanh", use_bias=False
                )
                w_u = tf.get_variable(
                    "w_u", shape=(self.msg_embed_size, 1), initializer=self.initializer
                )
            u = tf.reduce_mean(
                tf.tensordot(proj_u, w_u, axes=1), axis=-1
            )  # batch_size * max_n_days * max_n_msgs

            mask_msgs = tf.sequence_mask(
                self.n_msgs_ph, maxlen=self.max_n_msgs, dtype=tf.bool, name="mask_msgs"
            )
            ...
            corpus_embed = tf.matmul(
                u, self.msg_embed
            )  # batch_size * max_n_days * 1 * msg_embed_size
            corpus_embed = tf.reduce_mean(
                corpus_embed, axis=-2
            )  # batch_size * max_n_days * msg_embed_size
            self.corpus_embed = tf.nn.dropout(
                corpus_embed, keep_prob=1 - self.dropout_ce, name="corpus_embed"
            )
    ...
    def _build_mie(self):
        ...
        with tf.name_scope("mie"):
            ...
            if self.variant_type == "tech":
                ...
                cell = MSINCell(
                    input_size=self.price_size,
                    num_units=self.msin_h_size,
                    v_size=self.msg_embed_size,
    ...
    def _create_vmd_with_h_rec(self):
        with tf.name_scope("vmd"):
            with tf.variable_scope("vmd_h_rec"):
                ...
                y_ = tf.transpose(
                    self.y_ph, [1, 0, 2]
                )  # max_n_days * batch_size * y_size

                self.mask_aux_trading_days = tf.sequence_mask(
                    self.T_ph - 1,
                    self.max_n_days,
                    dtype=tf.bool,
                ...
                def _loop_body(t, ta_h_s, ta_z_prior, ta_z_post, ta_kl):
                    ...
                    with tf.variable_scope("iter_body", reuse=tf.AUTO_REUSE):
                        ...
                        def _init():
                            h_s_init = tf.nn.tanh(
                                tf.random_normal(shape=[self.batch_size, self.h_size])
                            )
                            h_z_init = tf.nn.tanh(
                                tf.random_normal(shape=[self.batch_size, self.z_size])
                            )
                        ...
                        def _subsequent():
                            h_s_t_1 = tf.reshape(
                                ta_h_s.read(t - 1), [self.batch_size, self.h_size]
                            )
                            z_t_1 = tf.reshape(
                                ta_z_post.read(t - 1), [self.batch_size, self.z_size]
                            )
                        ...
                        with tf.variable_scope("h_z_prior"):
                            h_z_prior_t = self._linear(
                                [x[t], h_s_t], self.z_size, "tanh"
                            )
                        with tf.variable_scope("z_prior"):
                            z_prior_t, z_prior_t_pdf = self._z(
                                h_z_prior_t, is_prior=True
                            )

                        with tf.variable_scope("h_z_post"):
                            h_z_post_t = self._linear(
                                [x[t], h_s_t, y_[t]], self.z_size, "tanh"
                            )
                    ...
                    ta_z_prior = ta_z_prior.write(
                        t, z_prior_t
                    )  # write: batch_size * z_size
                    ta_z_post = ta_z_post.write(
                        t, z_post_t
                    )  # write: batch_size * z_size
                ...
                ta_h_s_init = tf.TensorArray(
                    tf.float32, size=self.max_n_days, clear_after_read=False
                )
                ta_z_prior_init = tf.TensorArray(tf.float32, size=self.max_n_days)
                ta_z_post_init = tf.TensorArray(
                    tf.float32, size=self.max_n_days, clear_after_read=False
                )
                ...
                _, ta_h_s, ta_z_prior, ta_z_post, ta_kl = tf.while_loop(
                    loop_cond, _loop_body, loop_init
                )

                h_s = tf.reshape(
                    ta_h_s.stack(),
                    shape=(self.max_n_days, self.batch_size, self.h_size),
                )
                ...
                h_s = tf.transpose(
                    h_s, [1, 0, 2]
                )  # batch_size * max_n_days * vmd_h_size
                z_prior = tf.transpose(
                    z_prior, [1, 0, 2]
                )  # batch_size * max_n_days * z_size
                z_post = tf.transpose(
                    z_post, [1, 0, 2]
                )  # batch_size * max_n_days * z_size
                self.kl = tf.reduce_sum(
                    tf.transpose(kl, [1, 0, 2]), axis=2
                )  # batch_size * max_n_days

                with tf.variable_scope("g"):
                    self.g = self._linear(
                        [x, h_s, z_post], self.g_size, "tanh", use_bn=False
                    )

                with tf.variable_scope("y"):
                    self.y = self._linear(self.g, self.y_size, "softmax")

                sample_index = tf.reshape(
                    tf.range(self.batch_size), (self.batch_size, 1), name="sample_index"
                )
                self.indexed_T = tf.concat(
                    [sample_index, tf.reshape(self.T_ph - 1, (self.batch_size, 1))],
                    axis=1,
                )

                def _infer_func():
                    g_T = tf.gather_nd(
                        params=self.g, indices=self.indexed_T
                    )  # batch_size * g_size

                    if not self.daily_att:
                        y_T = tf.gather_nd(
                            params=self.y, indices=self.indexed_T
                        )  # batch_size * y_size
                        return g_T, y_T
                ...
                def _gen_func():
                    # use prior for g
                    z_prior_T = tf.gather_nd(
                        params=z_prior, indices=self.indexed_T
                    )  # batch_size * z_size
                    ...
                    with tf.variable_scope("g", reuse=tf.AUTO_REUSE):
                        g_T = self._linear(
                            [x_T, h_s_T, z_prior_T], self.g_size, "tanh", use_bn=False
                        )
                ...
                if not self.daily_att:
                    self.g_T, self.y_T = tf.cond(
                        tf.equal(self.is_training_phase, True), _infer_func, _gen_func
                    )
                else:
                    self.g_T = tf.cond(
                        tf.equal(self.is_training_phase, True), _infer_func, _gen_func
        ...
        with tf.name_scope("vmd"):
            with tf.variable_scope("vmd_zh_rec", reuse=tf.AUTO_REUSE):
                ...
                cell = tf.contrib.rnn.DropoutWrapper(
                    cell, output_keep_prob=1.0 - self.dropout_vmd
                )

                init_state = None
                # calculate vmd_h, batch_size * max_n_days * vmd_h_size
                h_s, _ = tf.nn.dynamic_rnn(
                    cell,
                    x,
                    sequence_length=self.T_ph,
                ...
                h_s = tf.transpose(
                    h_s, [1, 0, 2]
                )  # max_n_days * batch_size * vmd_h_size
                y_ = tf.transpose(
                    self.y_ph, [1, 0, 2]
                )  # max_n_days * batch_size * y_size
                ...
                def _loop_body(t, ta_z_prior, ta_z_post, ta_kl):
                    ...
                    with tf.variable_scope("iter_body", reuse=tf.AUTO_REUSE):
                        ...
                        init = lambda: tf.random_normal(
                            shape=[self.batch_size, self.z_size], name="z_post_t_1"
                        )
                        subsequent = lambda: tf.reshape(
                            ta_z_post.read(t - 1), [self.batch_size, self.z_size]
                        )
                        ...
                        with tf.variable_scope("h_z_prior"):
                            h_z_prior_t = self._linear(
                                [x[t], h_s[t], z_post_t_1], self.z_size, "tanh"
                            )
                        with tf.variable_scope("z_prior"):
                            z_prior_t, z_prior_t_pdf = self._z(
                                h_z_prior_t, is_prior=True
                        ...
                        with tf.variable_scope("h_z_post"):
                            h_z_post_t = self._linear(
                                [x[t], h_s[t], y_[t], z_post_t_1], self.z_size, "tanh"
                            )
                        with tf.variable_scope("z_post"):
                            z_post_t, z_post_t_pdf = self._z(h_z_post_t, is_prior=False)

                    kl_t = ds.kl_divergence(
                        z_post_t_pdf, z_prior_t_pdf
                    )  # batch_size * z_size
                ...
                _, ta_z_prior, ta_z_post, ta_kl = tf.while_loop(
                    cond, _loop_body, loop_init
                )
                ...
                with tf.variable_scope("g"):
                    self.g = self._linear(
                        [h_s, z_post], self.g_size, "tanh"
                    )  # batch_size * max_n_days * g_size

                with tf.variable_scope("y"):
                    self.y = self._linear(
                        self.g, self.y_size, "softmax"
                    )  # batch_size * max_n_days * y_size
                ...
                def _gen_func():
                    ...
                    with tf.variable_scope("g", reuse=tf.AUTO_REUSE):
                        g_T = self._linear(
                            [h_s_T, z_prior_T], self.g_size, "tanh", use_bn=False
                        )
            ...
            with tf.variable_scope("vmd_zh_rec", reuse=tf.AUTO_REUSE):
                ...
                def _loop_body(t, ta_z):
                    ...
                    with tf.variable_scope("iter_body", reuse=tf.AUTO_REUSE):
                        ...
                        subsequent = lambda: tf.reshape(
                            ta_z.read(t - 1), [self.batch_size, self.z_size]
                        )
                        ...
                        with tf.variable_scope("h_z"):
                            h_z_t = self._linear(
                                [x[t], h_s[t], z_t_1], self.z_size, "tanh"
                            )
        ...
        with tf.name_scope("tda"):
            with tf.variable_scope("tda"):
                with tf.variable_scope("v_i"):
                    ...
                    w_i = tf.get_variable(
                        "w_i", shape=(self.g_size, 1), initializer=self.initializer
                    )
                v_i = tf.reduce_sum(
                    tf.tensordot(proj_i, w_i, axes=1), axis=-1
                )  # batch_size * max_n_days
                ...
                self.v_stared = tf.where(
                    tf.is_nan(v_stared), tf.zeros_like(v_stared), v_stared
                )

                if self.daily_att == "y":
                    context = tf.transpose(
                        self.y, [0, 2, 1]
                    )  # batch_size * y_size * max_n_days
                else:
                    context = tf.transpose(
                        self.g, [0, 2, 1]
                    )  # batch_size * g_size * max_n_days

                v_stared = tf.expand_dims(
                    self.v_stared, -1
                )  # batch_size * max_n_days * 1
                att_c = tf.reduce_sum(
                    tf.matmul(context, v_stared), axis=-1
                )  # batch_size * g_size / y_size
    ...
    def _create_generative_ata(self):
        ...
        with tf.name_scope("ata"):
            with tf.variable_scope("ata"):
                ...
                minor = 0.0  # 0.0, 1e-7*
                likelihood_aux = tf.reduce_sum(
                    tf.multiply(self.y_ph, tf.log(self.y + minor)), axis=2
                )  # batch_size * max_n_days

                kl_lambda = self._kl_lambda()
                obj_aux = (
                    likelihood_aux - kl_lambda * self.kl
                )  # batch_size * max_n_days

                # deal with T specially, likelihood_T: batch_size, 1
                self.y_T_ = tf.gather_nd(
                    params=self.y_ph, indices=self.indexed_T
                )  # batch_size * y_size
                likelihood_T = tf.reduce_sum(
                    tf.multiply(self.y_T_, tf.log(self.y_T + minor)),
                    axis=1,
                    keep_dims=True,
                )

                kl_T = tf.reshape(
                    tf.gather_nd(params=self.kl, indices=self.indexed_T),
                    shape=[self.batch_size, 1],
                )
                obj_T = likelihood_T - kl_lambda * kl_T

                obj = obj_T + tf.reduce_sum(
                    tf.multiply(obj_aux, v_aux), axis=1, keep_dims=True
                )  # batch_size * 1
    ...
    def _create_discriminative_ata(self):
        ...
        with tf.name_scope("ata"):
            with tf.variable_scope("ata"):
                ...
                obj = likelihood_T + tf.reduce_sum(
                    tf.multiply(likelihood_aux, v_aux), axis=1, keep_dims=True
                )  # batch_size * 1
        ...
        with tf.name_scope("optimizer"):
            if self.opt == "sgd":
                ...
                optimizer = tf.train.MomentumOptimizer(
                    learning_rate=decayed_lr, momentum=self.momentum
                )
            else:
    ...
    def _kl_lambda(self):
        ...
        return tf.cond(
            global_step < self.kl_lambda_start_step, lambda: 0.0, _nonzero_kl_lambda
        )
    ...
    def _linear(self, args, output_size, activation=None, use_bias=True, use_bn=False):
        ...
        if use_bn:
            res = batch_norm(
                ...
                is_training=self.is_training_phase,
                scope=scope,
            )
