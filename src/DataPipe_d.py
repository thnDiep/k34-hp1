...
class DataPipe:
    ...
    def _get_prices_and_ts(self, ss, main_target_date):
        ...
        ts, ys, prices, mv_percents, main_mv_percent = (
            list(),
            list(),
            list(),
            list(),
        ...
        with io.open(stock_movement_path, "r", encoding="utf8") as movement_f:
            for line in movement_f:  # descend
                ...
                if t == main_target_date:
                    ...
                    if (
                        -0.005 <= float(main_mv_percent) < 0.0055
                    ):  # discard sample with low movement percent
                        return None
        ...
        T = len(ts)
        if (
            len(ys) != T or len(prices) != T or len(mv_percents) != T
    ...
    def _get_unaligned_corpora(self, ss, main_target_date, vocab_id_dict):
        def get_ss_index(word_seq, ss):
            ...
            if ss in word_seq:
                ...
                if "$" in word_seq:
                    ...
                    if (
                        dollar_index is not len(word_seq) - 1
                        and ss in word_seq[dollar_index + 1]
                    ):
        ...
        d = d_d_max  # descend
        while d >= d_d_min:
            msg_fp = os.path.join(stock_tweet_path, d.isoformat() + ".json")
    ...
    def _trading_day_alignment(self, ts, T, unaligned_corpora):
        aligned_word_tensor = np.zeros(
            [T, self.max_n_msgs, self.max_n_words], dtype=np.int32
        )
        ...
        for t in range(T):
            ...
            if aligned_msgs[t] and aligned_ss_indices[t] and aligned_n_words[t]:
                msgs, ss_indices, n_word = (
                    np.vstack(aligned_msgs[t]),
                    np.hstack(aligned_ss_indices[t]),
                    np.hstack(aligned_n_words[t]),
                )
    ...
    def sample_gen_from_one_stock(self, vocab_id_dict, stock_id_dict, s, phase):
        ...
        for main_target_date in main_target_dates:
            ...
            unaligned_corpora = self._get_unaligned_corpora(
                s, main_target_date, vocab_id_dict
            )
            ...
            aligned_info_dict = self._trading_day_alignment(
                prices_and_ts["ts"], prices_and_ts["T"], unaligned_corpora
            )
    ...
    def batch_gen(self, phase):
        ...
        generators = [
            self.sample_gen_from_one_stock(vocab_id_dict, stock_id_dict, s, phase)
        ...
        while True:
            ...
            y_batch = np.zeros(
                [batch_size, self.max_n_days, self.y_size], dtype=np.float32
            )
            main_mv_percent_batch = np.zeros(
                [
                    batch_size,
                ],
                dtype=np.float32,
            ...
            word_batch = np.zeros(
                [batch_size, self.max_n_days, self.max_n_msgs, self.max_n_words],
                dtype=np.int32,
            )
            ss_index_batch = np.zeros(
                [batch_size, self.max_n_days, self.max_n_msgs], dtype=np.int32
            )
            n_msgs_batch = np.zeros([batch_size, self.max_n_days], dtype=np.int32)
            n_words_batch = np.zeros(
                [batch_size, self.max_n_days, self.max_n_msgs], dtype=np.int32
            )
    ...
    def init_word_table(self):
        word_table_init = (
            np.random.random((vocab_size, self.word_embed_size)) * 2 - 1
        )  # [-1.0, 1.0]
